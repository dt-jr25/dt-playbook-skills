---
description: Runs the Dynatrace logs overview playbook end-to-end against the active dtctl context. Produces a tenant-wide state-of-the-logs report (volume, top emitters, severity mix, ingest sources, data dictionary, top error signatures, PII checks, anomalies, parse-rule and metric opportunities) and writes it to `OUTPUT/<context-folder>/reports/logs-overview-<YYYY-MM-DD-HHMM>.md`. Invoke with `@dt-logs-review` whenever the user asks for a logs overview, logs review, logs report, or "what logs are we capturing". Always reads the `dt-playbook-common` skill first for the Step 0 kickoff interview and intent confirmation before any dtctl query. Uses 1-in-1000 sampling on 24 h discovery queries to cap Grail scan cost.
---
...
2. **Read the `dt-logs-review` skill next.** Follow its Discovery Query Set (§1–§12), §Query guardrails (the 1-in-1000 sampling rules for 24 h queries), §What-to-Look-For checklist, and §Output Document Structure verbatim. Honour its parameter table (`<kind>=reports/`, `<filename-stem>=logs-overview`, `<records>=logs`).
...
6. **Step 3 — deep dive + write report.** Run §8–§11, build the report per §Output Document Structure (labelling sampled counts as estimates and `countDistinct` values as lower bounds), write to `OUTPUT/<context-folder>/reports/logs-overview-<YYYY-MM-DD-HHMM>.md` (UTC, never overwrite).
